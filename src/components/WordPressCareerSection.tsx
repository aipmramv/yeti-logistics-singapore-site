...
import { useWordPressJobs } from '@/hooks/useWordPress';

const WordPressCareerSection = () => {
  const { data: jobs, loading, error } = useWordPressJobs();

  // Fallback jobs
  const fallbackJobs = [
    {
      title: { rendered: "Class 3 Drivers" },
      acf: {
        type: "Full-time",
        location: "Singapore",
    ...
    {
      title: { rendered: "Warehouse Packers" },
      acf: {
        type: "Full-time",
        location: "Singapore",
    ...
    {
      title: { rendered: "Logistics Coordinator" },
      acf: {
        type: "Full-time",
        location: "Singapore",
        department: "Operations",
  ...
  const displayJobs = (jobs && jobs.length > 0) ? jobs.filter(job => job.acf?.published) : fallbackJobs;
          ...
          {displayJobs.map((job, index) => (
            <Card key={index} className="group hover:shadow-xl transition-all duration-300 border-0 shadow-lg">
              <CardContent className="p-6">
                ...
                <Button 
                  onClick={() => handleApply(job.title.rendered)}
...
export default WordPressCareerSection;
