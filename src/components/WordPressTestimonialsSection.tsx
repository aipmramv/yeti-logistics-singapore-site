...
import { useWordPressTestimonials } from '@/hooks/useWordPress';

const WordPressTestimonialsSection = () => {
  const { data: testimonials, loading, error } = useWordPressTestimonials();
        ...
        <Card className="bg-white/10 backdrop-blur-sm border-0 shadow-2xl">
          <CardContent className="p-12 text-center">
            ...
            <blockquote className="text-xl sm:text-2xl font-medium leading-relaxed mb-8 text-white">
              "{displayTestimonials[currentTestimonial]?.acf?.message}"
            </blockquote>
            
            <div className="border-t border-white/20 pt-6">
              <div className="font-bold text-lg text-blue-200">
                {displayTestimonials[currentTestimonial]?.acf?.name}
              </div>
              <div className="text-blue-300">
                {displayTestimonials[currentTestimonial]?.acf?.position}
              </div>
              <div className="text-blue-400 text-sm">
                {displayTestimonials[currentTestimonial]?.acf?.company}
              </div>
            </div>
          </CardContent>
        </Card>
...
export default WordPressTestimonialsSection;
